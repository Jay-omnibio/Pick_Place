...
class ActiveInferenceAgent:
    """
    Active-inference pick-and-place agent loop:
    sense -> infer -> BT -> action select -> act
    ...
    def _build_task_cfg_fallback(active_cfg):
        ...
        return SimpleNamespace(
            ...
            use_object_local_xy_errors=True,
            reach_xy_threshold=float(cfg.get("approach_threshold", 0.03)),
            reach_z_threshold=float(cfg.get("approach_threshold", 0.03)),
            descend_threshold=float(cfg.get("descend_threshold", 0.02)),
            descend_x_threshold=float(cfg.get("descend_x_threshold", 0.01)),
            descend_y_threshold=float(cfg.get("descend_y_threshold", 0.01)),
            descend_z_threshold=float(cfg.get("descend_z_threshold", 0.01)),
            descend_contact_z_threshold=float(cfg.get("descend_timeout_z_threshold", 0.03)),
            descend_timeout_x_threshold=float(cfg.get("descend_timeout_x_threshold", 0.03)),
            descend_timeout_y_threshold=float(cfg.get("descend_timeout_y_threshold", 0.03)),
            descend_timeout_z_threshold=float(cfg.get("descend_timeout_z_threshold", 0.03)),
            descend_max_steps=int(cfg.get("descend_max_steps", 1000)),
            descend_stop_contact_steps=1,
            descend_ready_steps=1,
            stable_contact_steps=int(cfg.get("grasp_stable_steps_for_lift", 1)),
            close_hold_steps=int(cfg.get("close_hold_steps", 12)),
            lift_test_steps=int(cfg.get("lift_test_steps", 10)),
            open_hold_steps=int(cfg.get("open_hold_steps", 35)),
            retreat_steps=int(cfg.get("retreat_steps", 24)),
            preplace_threshold=float(cfg.get("preplace_threshold", 0.04)),
            place_threshold=float(cfg.get("place_threshold", 0.03)),
            preplace_xy_threshold=float(cfg.get("preplace_xy_threshold", 0.02)),
            preplace_z_threshold=float(cfg.get("preplace_z_threshold", 0.03)),
            place_xy_threshold=float(cfg.get("place_xy_threshold", 0.02)),
            place_z_threshold=float(cfg.get("place_z_threshold", 0.015)),
            transit_height=float(cfg.get("transit_height", 0.35)),
        )
    ...
    def __init__(
        ...
        self.step_count = 0
        if active_inference_cfg is None:
            raise ValueError(
                "active_inference_cfg is required in active-inference mode. "
                "Load it from config/active_inference_config.yaml."
            )
        self.active_inference_cfg = dict(active_inference_cfg or {})
        self.task_cfg = task_cfg if task_cfg is not None else self._build_task_cfg_fallback(self.active_inference_cfg)
        self.current_belief = None
        self.ai_startup_settle_steps = max(0, int(os.getenv("AI_STARTUP_SETTLE_STEPS", "80")))
        self.active_inference_params = self._build_active_inference_params()
        self.ai_bt = AIPickPlaceBehaviorTree(
    ...
    def _update_ai_risk_detection(self, observation):
        if self.current_belief is None or not self.ai_risk_detection_enabled:
    ...
    def _collect_log_metrics(self, sim_state, observation):
        ...
        if bool(self.active_inference_params.get("use_world_place_goal_pose", False)):
            goal_world = self._ai_goal_world_xyz()
    ...
    def _log_heartbeat(self, sim_state, observation, action):
        ...
        if phase == "Reach" and self.current_belief is not None:
            ...
            ai_descend_x_thr = float(self.active_inference_params.get("descend_x_threshold", 0.01))
            ai_descend_y_thr = float(self.active_inference_params.get("descend_y_threshold", 0.01))
            ai_descend_z_thr = float(self.active_inference_params.get("descend_z_threshold", 0.01))
            ai_descend_thr = float(self.active_inference_params.get("descend_threshold", 0.02))
            ...
            close_step = int(self.current_belief.get("close_hold_timer", 0)) if self.current_belief else 0
            close_steps = int(self.active_inference_params.get("close_hold_steps", 0))
            stable_contact = int(self.current_belief.get("stable_grasp_counter", 0)) if self.current_belief else 0
            stable_need = int(self.active_inference_params.get("grasp_stable_steps_for_lift", 0))
            msg += (
                f" x={m['true_descend_x_error']:.4f} "
                f"y={m['true_descend_y_error']:.4f} "
                f"z={m['true_descend_z_error']:.4f} "
                f" close_step={close_step}/{close_steps} "
                f"stable_contact={stable_contact}/{stable_need}"
            )
        elif phase == "LiftTest":
            lift_step = int(self.current_belief.get("lift_test_timer", 0)) if self.current_belief else 0
            lift_steps = int(self.active_inference_params.get("lift_test_steps", 0))
            msg += (
                f" lift_step={lift_step}/{lift_steps} "
            ...
            pre_xy_thr = float(self.active_inference_params.get("preplace_xy_threshold", 0.02))
            pre_z_thr = float(self.active_inference_params.get("preplace_z_threshold", 0.03))
            pre_thr = float(self.active_inference_params.get("preplace_threshold", 0.04))
            msg += (
            ...
            place_xy_thr = float(self.active_inference_params.get("place_xy_threshold", 0.02))
            place_z_thr = float(self.active_inference_params.get("place_z_threshold", 0.015))
            place_thr = float(self.active_inference_params.get("place_threshold", 0.03))
            msg += (
        ...
        elif phase == "Open" and self.current_belief is not None:
        ...
        elif phase == "Retreat" and self.current_belief is not None:
    ...
    def _maybe_escape_stall(self, action, sim_state):
        if not self.ai_allow_lowlevel_escape:
            self.escape_active = 0
            self.recovery_steps_remaining = 0
            return action
    ...
    def _maybe_pause_for_inspection(self, sim_state, observation):
        ...
        descend_y_error = float(abs((obj_rel_eval - descend_ref)[1]))
        descend_z_error = float(abs((obj_rel_eval - descend_ref)[2]))
        phase = self.current_belief.get("phase", "Reach") if self.current_belief is not None else "Reach"
        ee_z = float(ee_pos[2])
    ...
    def _log_events(self, sim_state, observation, action):
        ...
        if phase != self.prev_phase:
            ...
            if self.prev_phase == "Descend" and phase == "CloseHold":
                close_step = int(self.current_belief.get("close_hold_timer", 0)) if self.current_belief else 0
                close_steps = int(self.active_inference_params.get("close_hold_steps", 0))
                msg += f" close_step={close_step}/{close_steps}"
            elif phase == "Descend":
            ...
            elif phase == "Failure" and self.current_belief is not None:
